...
# 生成的文件的地址前面加上7_P6/test/
# VSCode中运行Python的工作目录和VSCode的工作目录是相同的
os.system("java -jar test/mars.jar a mc CompactLargeText nc dump .text HexText mips/code.txt test/test_code/P6_test_code_" + test_file_number + ".asm")
...
# 使用MARS运行，得到标准结果
os.system("java -jar test/mars.jar 10000 mc CompactLargeText nc coL1 db ig test/test_code/P6_test_code_" + test_file_number + ".asm > test/test_code/P6_mars_result_" + test_file_number + ".txt")
...
if (step == 2):
    ...
    os.system("cd mips/; ./mips_txt_isim_beh.exe -nolog -tclbatch mips_txt.tcl > ../test/test_code/P6_ise_result_" + test_file_number + ".txt")

    # 对比
    marsresultfile = open("./test/test_code/P6_mars_result_" + test_file_number + ".txt", mode="r")
    iseresultfile = open("./test/test_code/P6_ise_result_" + test_file_number + ".txt", mode="r")
    ...
    if(iselen == marslen):
        print("\033[0;32mLines match!\n ISE Lines: {:>5d}\n".format(iselen) + "MARS Lines: {:>5d}".format(marslen) + "\033[0m")
    else:
